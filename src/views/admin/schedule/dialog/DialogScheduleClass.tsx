...
interface DialogScheduleClassProps {
  dialogOpen: boolean;
...
// A simple counter to ensure unique IDs for each dialog instance.
let dialogCounter = 0;
const generateUniqueDialogId = () => `dialog-schedule-class-${dialogCounter++}`;


function DialogScheduleClass({ dialogOpen, setDialogOpen, selectedCell }: DialogScheduleClassProps) {
  ...
  // --- 2. GENERATE AND STORE UNIQUE ID ---
  // The unique ID is generated once when the component is initialized.
  const [uniqueId] = useState(generateUniqueDialogId);
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
      {/* --- 3. APPLY THE UNIQUE CLASS NAME --- */}
      <DialogContent className={`sm:max-w-[480px] p-0 bg-white rounded-xl shadow-2xl ${uniqueId}`}>
...
export default DialogScheduleClass;