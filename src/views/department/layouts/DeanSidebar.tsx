...
function DeanSidebar({ isOpen, setIsOpen, isCollapsed }: SidebarProps) {
  ...
  const isRouteActive = (pathname: string, href: string) => {
    ...
    return cleanPath.startsWith(href); // Gigamit ang startsWith para ma-highlight gihapon bisan naay sub-routes
  };
                  ...
                  <li key={link.href}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Link to={link.href} className={`flex items-center gap-3 p-3 rounded-lg text-base transition ${isRouteActive(location.pathname, link.href) ? "bg-white/10 font-medium text-white" : "text-gray-300 hover:bg-white/5"} ${hideText ? 'justify-center' : ''}`}>
