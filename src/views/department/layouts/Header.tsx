...
// --- 1. Gidugang ang bag-ong mga icon para sa Dean ---
import {
    BarChart3, ChevronDown, User, LogOut, Bell, Megaphone, CalendarCheck,
    FileText, Settings, Users, ClipboardCheck, AreaChart
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link, useNavigate } from 'react-router-dom';
import { toast } from 'sonner';

// --- Dummy Data para sa Notifications sa Dean ---
const dummyNotifications = [
    { id: 1, type: 'faculty_request', title: 'Prof. Reyes submitted a leave request.', timestamp: '30 minutes ago', read: false },
    { id: 2, type: 'approval', title: 'New Course Proposal: "Advanced AI" needs your approval.', timestamp: '4 hours ago', read: false },
    { id: 3, type: 'report', title: 'Q3 Department Performance report is now available.', timestamp: '1 day ago', read: true },
    { id: 4, type: 'budget', title: 'Reminder: Budget submission deadline is this Friday.', timestamp: '2 days ago', read: false },
    { id: 5, type: 'announcement', title: 'University-wide curriculum review meeting next week.', timestamp: '4 days ago', read: true },
];
...
function Header() {
  ...
  // --- Gihimong dinamiko ang user info para sa Dean ---
  const user = userString ? JSON.parse(userString) : { name: 'Dr. Elena Cruz', email: 'dean.cruz@university.edu' };
  ...
  const NotificationIcon = ({ type }: { type: string }) => {
    switch (type) {
        ...
        case 'faculty_request': return <User className="text-white"/>;
        case 'approval': return <ClipboardCheck className="text-white"/>;
        case 'report': return <FileText className="text-white"/>;
        case 'budget': return <CalendarCheck className="text-white"/>;
        default: return <Bell className="text-white"/>;
    }
  };
  const iconBgColor: Record<string, string> = {
      announcement: 'bg-rose-500',
      faculty_request: 'bg-sky-500',
    ...
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200">
      <div className="flex items-center justify-between py-3 px-4 md:px-6">
        <div className="flex items-center gap-3">
          {/* --- 2. Gi-ilisdan ang title sa portal --- */}
          <Link to="/facultyscheduler/dean/dashboard" className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold text-gray-800 tracking-wide">Dean's User</span>
          </Link>
        </div>
                    ...
                    {isNotifOpen && (
                        ...
                        >
                           ...
                           <div className="text-center p-2 bg-gray-50">
                                <Link to="/facultyscheduler/dean/notifications" onClick={() => setIsNotifOpen(false)} className="text-sm font-bold text-purple-600 hover:underline">
                ...
                <button onClick={() => { setIsProfileOpen(prev => !prev); setIsNotifOpen(false); }} className="flex items-center gap-2 focus:outline-none group">
                   <img src={`https://i.pravatar.cc/150?u=${user.email}`} alt="Profile" className="w-10 h-10 rounded-full border-2 border-transparent group-hover:border-purple-400 transition-colors duration-300"/>
                    <div className="hidden md:flex flex-col items-start">
                        ...
                        {/* --- 3. Gi-ilisdan ang role ngadto sa "Department Dean" --- */}
                        <span className="text-xs text-gray-500">Department Dean</span>
                    </div>
                ...
                <AnimatePresence>
                    {isProfileOpen && (
                        ...
                        >
                            ...
                            <div className="p-2">
                                <Link to="/facultyscheduler/dean/profile" onClick={() => setIsProfileOpen(false)} className="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-gray-100 transition-colors">
                                    <User size={16} /><span>My Profile</span>
                                </Link>
                                <Link to="/facultyscheduler/dean/settings" onClick={() => setIsProfileOpen(false)} className="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-gray-100 transition-colors">
                                    <Settings size={16} /><span>Settings</span>
                                </Link>

                                {/* --- 4. GIDUGANG NGA MGA LINK PARA SA DEAN --- */}
                                <hr className="border-t border-gray-200/80 my-1" />
                                <Link to="/facultyscheduler/dean/faculty-overview" onClick={() => setIsProfileOpen(false)} className="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-gray-100 transition-colors">
                                    <Users size={16} /><span>Faculty Overview</span>
                                </Link>
                                <Link to="/facultyscheduler/dean/course-approvals" onClick={() => setIsProfileOpen(false)} className="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-gray-100 transition-colors">
                                    <ClipboardCheck size={16} /><span>Course Approvals</span>
                                </Link>
                                <Link to="/facultyscheduler/dean/analytics" onClick={() => setIsProfileOpen(false)} className="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 hover:bg-gray-100 transition-colors">
                                    <AreaChart size={16} /><span>Department Analytics</span>
                                </Link>
                                <hr className="border-t border-gray-200/80 my-1" />
